function [step_indices] = march_step_detection(data_directory, rise_indices, fall_indices, fb_step_indices, show_plots, export_steps)
...
[accel_filt_y, accel_y_cut_freq]  = apply_adapted_LPF(accel_data.y, f, fs);
[accel_filt_x] = apply_adapted_LPF(accel_data.x, f, fs);
[accel_filt_z, accel_z_cut_freq] = apply_adapted_LPF(accel_data.z, f, fs);
...
chosen_cut_freq = min(abs(accel_z_cut_freq), abs(accel_y_cut_freq));
for rise_i=1:length(rise_indices)
    sequence = accel_filt_y(rise_indices(rise_i):fall_indices(rise_i));
    min_peak_distance = (1/chosen_cut_freq)*fs / 2;

    [~, peak_locs] = findpeaks(sequence, "MinPeakDistance", min_peak_distance, "MinPeakHeight", max(sequence)/3, "MinPeakProminence", max(sequence)/1.25);
...
%% remove duplicates with fb steps indices
same_step_threshold = 150;
march_cand_white_list = zeros(step_count, 1);
step_counter = 1;
for march_step_index = step_indices(1:step_count)'
    if all(abs(fb_step_indices - march_step_index) > same_step_threshold)
        march_cand_white_list(step_counter) = march_step_index;
        step_counter = step_counter + 1;
    end
end
step_indices = march_cand_white_list;
step_count = step_counter - 1;
...
if export_steps
    ...
    for step_i=1:step_count
        gyro_steps(step_i, 1:step_lengths(step_i)) = gyro_filt_z(step_indices(step_i):step_indices(step_i)+step_lengths(step_i)-1);
    end
    accel_z_fft = fft(steps, 50, 2);
    step_indices = step_indices(1:step_count);
    
    export_mat = [
        step_indices, ... # step start index
        step_sequence_indexes(1:step_count), ... # sequence start index
        steps(1:step_count, 1:100), ... # z-acceleration step samples
        abs(accel_z_fft(1:step_count, :)), ... # z-accelerometer step samples FFT
        gyro_steps(:, 1:100), ... # z-gyro step samples
        accel_filt_z(step_sequence_indexes(1:step_count)), ... # sequence start z-acceleration
        mean(abs(steps(1:step_count, :)).^2, 2), ... # z-acceleration mean power
        repmat(abs(accel_z_cut_freq), step_count, 1), ... # z-acceleration cut frequency    
        accel_filt_x(step_indices), ... # step start x-acceleration 
        accel_filt_y(step_indices), ... # step start y-acceleration 
        gyro_filt_x(step_indices), ... # step start x-gyro
        gyro_filt_y(step_indices) ... # step start y-gyro
    ];
    writematrix(export_mat, 'step_data_m.csv');
end
